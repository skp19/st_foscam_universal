...
metadata {
	...
	tiles {
        ...
        standardTile("ledAuto", "device.ledStatus", width: 1, height: 1, canChangeIcon: false, inactiveLabel: true, canChangeBackground: false) {
          ...
          state "on", label: "on", action: "ledAuto", icon: "st.Lighting.light11", backgroundColor: "#FFFFFF"
          state "manual", label: "manual", action: "ledAuto", icon: "st.Lighting.light13", backgroundColor: "#FFFFFF"
        }
...
def parse(String description) {
    ...
    log.debug "Bucket: " + descMap["bucket"]
    log.debug "Key: " + descMap["key"]
	...
	//Status Polling
        ...
        if(hdcamera == true) {
            ...
            log.debug "MOTION ALARM: $motionAlarm"
            def ledMode = "$langs.mode"
            log.debug "LED MODE: $ledMode"
